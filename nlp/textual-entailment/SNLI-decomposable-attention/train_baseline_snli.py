...
#             para_norm ** 0.5
            ...
            correct += torch.sum(predict == train_lbl_batch.data).item()
            loss_data += (loss.data[0] * batch_size).item()  # / train_lbl_batch.data.size()[0])
        ...
        if (k + 1) % args.dev_interval == 0:
            ...
            for i in range(len(dev_batches)):
                ...
                correct += torch.sum(predict == dev_lbl_batch.data).item()
