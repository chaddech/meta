...
full_train_idx = np.load('new_smaller_MNIST_10000_train_indices.npy')
full_valid_idx = np.load('new_smaller_MNIST_10000_valid_indices.npy')
meta_full_train_idx = np.load('new_smaller_MNIST_10000_meta_meta_train_indices.npy')
meta_full_valid_idx = np.load('new_smaller_MNIST_10000_meta_meta_valid_indices.npy')
meta_full_test_idx = np.load('new_smaller_MNIST_10000_meta_meta_test_indices.npy')
...
name_and_time = str(sys.argv[0]) + str(datetime.datetime.now())+"fclayer128"
...
#           self.X_data.append(torch.load(correct_files[i]))
    ...
    def __getitem__(self, idx):
        ...
        for layer in range(len(self.X_data)):
            ...
            Xs_to_return.append(self.X_data[layer][idx])
...
def test_meta_model(model,device,error_test_loader, correct_test_loader,optimizer,epoch):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(correct_test_loader):
            ...
            output = model(data[0])
            target = data[1].view(-1, data[1].shape[0])[0]
            loss = F.nll_loss(output, target)
            test_loss += loss
...
def meta_fc1_hook(self, input, output):
    meta_fc1_output.append(output.data)

def meta_fc4_hook(self, input, output):
    meta_fc4_output.append(output.data)

def meta_fc5_hook(self, input, output):
    meta_fc5_output.append(output.data)

def meta_fc6_hook(self, input, output):
    meta_fc6_output.append(output.data)
...
def test_predicting_meta(model, device, train_loader, optimizer, epoch):
    model.eval()
    correct = 0
    with torch.no_grad():
...
def makeIntermediates(args, model, device, test_loader, mode):
    ...
    list_of_incorrect_targets = []
    list_of_correct_targets = []
    ...
    print("Generating " + mode + " intermediate")
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            ...
            incorrect_predictions = pred[where_wrong]
            incorrect_target_batch = target[where_wrong]
            ...
            correct_predictions = pred[where_correct]
            correct_target_batch = target[where_correct]
            #correct_targets = target[where_correct]
            ...
            for i in incorrect_target_batch:
                incorrect_targets.append(i)

            for i in correct_target_batch:
                correct_targets.append(i)
        ...
        correct_fc1_outputs = list(zip(correct_fc1_outputs, correct_targets, correct_preds))
        incorrect_fc1_outputs = list(zip(incorrect_fc1_outputs, incorrect_targets, incorrect_preds))
        correct_conv1_outputs = list(zip(correct_conv1_outputs, correct_targets, correct_preds))
        incorrect_conv1_outputs = list(zip(incorrect_conv1_outputs, incorrect_targets, incorrect_preds))
        correct_conv2_outputs = list(zip(correct_conv2_outputs, correct_targets, correct_preds))
        incorrect_conv2_outputs = list(zip(incorrect_conv2_outputs, incorrect_targets, incorrect_preds))
        correct_outputs = list(zip(correct_outputs, correct_targets, correct_preds))
        incorrect_outputs = list(zip(incorrect_outputs, incorrect_targets, incorrect_preds))
        ...
        torch.save(incorrect_preds, results_folder + 'incorrect_predictions_meta_' + mode +'.torch')
        torch.save(correct_preds, results_folder + 'correct_predictions_meta_' + mode +'.torch')
        torch.save(incorrect_targets, results_folder + 'incorrect_targets_meta_' + mode +'.torch')
        torch.save(correct_targets, results_folder + 'correct_targets_meta_' + mode +'.torch')
...
def makeMetaIntermediates(args, meta_model, device, meta_loader, layer, mode):
    meta_model.eval()
    ...
    list_of_correct_fc1_outputs = []
    list_of_incorrect_fc1_outputs = []
    list_of_correct_fc4_outputs = []
    list_of_incorrect_fc4_outputs = []
    list_of_correct_fc5_outputs = []
    list_of_incorrect_fc5_outputs = []
    list_of_correct_fc6_outputs = []
    list_of_incorrect_fc6_outputs = []
    ...
    meta_model.fc1.register_forward_hook(meta_fc1_hook)
    meta_model.fc4.register_forward_hook(meta_fc4_hook)
    meta_model.fc5.register_forward_hook(meta_fc5_hook)
    meta_model.fc6.register_forward_hook(meta_fc6_hook)

    print("Generating " + mode + " intermediate")


    outputs_correct_train_file = results_folder + 'correct_outputs_meta_meta_train.torch'
    outputs_incorrect_train_file = results_folder + 'incorrect_outputs_meta_meta_train.torch'
    conv2_correct_train_file = results_folder + 'conv2_inter_correct_meta_meta_train.torch'
    conv2_incorrect_train_file = results_folder + 'conv2_inter_incorrect_meta_meta_train.torch'
    conv1_correct_train_file = results_folder + 'conv1_inter_correct_meta_meta_train.torch'
    conv1_incorrect_train_file = results_folder + 'conv1_inter_incorrect_meta_meta_train.torch'

    fc1_correct_train_file = results_folder + 'fc1_inter_correct_meta_meta_train.torch'
    fc1_incorrect_train_file = results_folder + 'fc1_inter_incorrect_meta_meta_train.torch'

    fc1_correct_valid_file = results_folder + 'fc1_inter_correct_meta_meta_valid.torch'
    fc1_incorrect_valid_file = results_folder + 'fc1_inter_incorrect_meta_meta_valid.torch'

    conv2_correct_valid_file = results_folder + 'conv2_inter_correct_meta_meta_valid.torch'
    conv2_incorrect_valid_file = results_folder + 'conv2_inter_incorrect_meta_meta_valid.torch'

    conv1_correct_valid_file = results_folder + 'conv1_inter_correct_meta_meta_valid.torch'
    conv1_incorrect_valid_file = results_folder + 'conv1_inter_incorrect_meta_meta_valid.torch'

    fc1_correct_test_file = results_folder + 'fc1_inter_correct_meta_meta_test.torch'
    fc1_incorrect_test_file = results_folder + 'fc1_inter_incorrect_meta_meta_test.torch'

    conv2_correct_test_file = results_folder + 'conv2_inter_correct_meta_meta_test.torch'
    conv2_incorrect_test_file = results_folder + 'conv2_inter_incorrect_meta_meta_test.torch'
    conv1_correct_test_file = results_folder + 'conv1_inter_correct_meta_meta_test.torch'
    conv1_incorrect_test_file = results_folder + 'conv1_inter_incorrect_meta_meta_test.torch'

    outputs_correct_valid_file = results_folder + 'correct_outputs_meta_meta_valid.torch'
    outputs_incorrect_valid_file = results_folder + 'incorrect_outputs_meta_meta_valid.torch'

    outputs_correct_test_file = results_folder + 'correct_outputs_meta_meta_test.torch'
    outputs_incorrect_test_file = results_folder + 'incorrect_outputs_meta_meta_test.torch'
    ...
    if layer is "fc1":
        ...
        train_correct_files = [outputs_correct_train_file]
        train_error_files = [outputs_incorrect_train_file]

        valid_correct_files = [outputs_correct_valid_file]
        valid_error_files = [outputs_incorrect_valid_file]

        test_correct_files = [outputs_correct_test_file]
        test_error_files = [outputs_incorrect_test_file]

    elif layer is "conv2":
        train_correct_files = [conv2_correct_train_file]
        train_error_files = [conv2_incorrect_train_file]

        valid_correct_files = [conv2_correct_valid_file]
        valid_error_files = [conv2_incorrect_valid_file]

        test_correct_files = [conv2_correct_test_file]
        test_error_files = [conv2_incorrect_test_file]

    elif layer is "conv1":
        train_correct_files = [conv1_correct_train_file]
        train_error_files = [conv1_incorrect_train_file]

        valid_correct_files = [conv1_correct_valid_file]
        valid_error_files = [conv1_incorrect_valid_file]

        test_correct_files = [conv1_correct_test_file]
        test_error_files = [conv1_incorrect_test_file]
    ...
    with torch.no_grad():
        ...
        incorrect_fc1_outputs = []
        correct_fc4_outputs = []
        incorrect_fc4_outputs = []
        correct_fc5_outputs = []
        incorrect_fc5_outputs = []
        correct_fc6_outputs = []
        incorrect_fc6_outputs = []
        ...
        for data, target in meta_loader:
            ...
            incorrect_meta_fc1_outs = meta_fc1_output[0][where_wrong]
            incorrect_meta_fc4_outs = meta_fc1_output[0][where_wrong]
            incorrect_meta_fc5_outs = meta_fc1_output[0][where_wrong]
            incorrect_meta_fc6_outs = meta_fc1_output[0][where_wrong]
            ...
            correct_meta_fc1_outs = meta_fc1_output[0][where_correct]
            correct_meta_fc4_outs = meta_fc1_output[0][where_correct]
            correct_meta_fc5_outs = meta_fc1_output[0][where_correct]
            correct_meta_fc6_outs = meta_fc1_output[0][where_correct]

            for i in incorrect_outs:
                incorrect_outputs.append(i)

            for i in incorrect_meta_fc1_outs:
                incorrect_fc1_outputs.append(i)

            for i in incorrect_meta_fc4_outs:
                incorrect_fc4_outputs.append(i)

            for i in incorrect_meta_fc5_outs:
                incorrect_fc5_outputs.append(i)

            for i in incorrect_meta_fc6_outs:
                incorrect_fc6_outputs.append(i)


            for i in correct_outs:
                correct_outputs.append(i)

            for i in correct_meta_fc1_outs:
                correct_fc1_outputs.append(i)

            for i in correct_meta_fc4_outs:
                correct_fc4_outputs.append(i)

            for i in correct_meta_fc5_outs:
                correct_fc5_outputs.append(i)

            for i in correct_meta_fc6_outs:
                correct_fc6_outputs.append(i)
            ...
            meta_fc6_output.clear()
            meta_fc5_output.clear()
            meta_fc4_output.clear()
            meta_fc1_output.clear()
        ...
        torch.save(correct_fc1_outputs, results_folder + 'meta_fc1_inter_correct_meta_' + layer + mode +'.torch')
        torch.save(incorrect_fc1_outputs, results_folder + 'meta_fc1_inter_incorrect_meta_'  + layer+ mode +'.torch')
        torch.save(correct_fc4_outputs, results_folder + 'meta_fc4_inter_correct_meta_' + layer + mode +'.torch')
        torch.save(incorrect_fc4_outputs, results_folder + 'meta_fc4_inter_incorrect_meta_'  + layer+ mode +'.torch')
        torch.save(correct_fc5_outputs, results_folder + 'meta_fc5_inter_correct_meta_' + mode  + layer+'.torch')
        torch.save(incorrect_fc5_outputs, results_folder + 'meta_fc5_inter_incorrect_meta_' + layer + mode +'.torch')
        torch.save(correct_fc6_outputs, results_folder + 'meta_fc6_inter_correct_meta_'  + layer+ mode +'.torch')
        torch.save(incorrect_fc6_outputs, results_folder + 'meta_fc6_inter_incorrect_meta_' + layer + mode +'.torch')

        torch.save(correct_outputs, results_folder + 'meta_correct_outputs_meta_' + mode +'.torch')
        torch.save(incorrect_outputs, results_folder + 'meta_incorrect_outputs_meta_' + mode +'.torch')
...
def make_and_train_base_model(args, train_dataset, device):
    ...
    validation_sampler = SubsetRandomSampler(full_valid_idx)
    ...
    for epoch in range(args.base_train_num_epochs):
        ...
        test_base_model(args, base_model, device, base_validation_loader, epoch, "valid", len(full_valid_idx))

    torch.save(base_model.state_dict(), results_folder + "base_model.torch")

    return base_model

def make_and_train_meta_model(args, device, train_set_percentage, layer):
    ...
    conv2_incorrect_train_file = results_folder + 'conv2_inter_incorrect_meta_train.torch'
    conv1_correct_train_file = results_folder + 'conv1_inter_correct_meta_train.torch'
    conv1_incorrect_train_file = results_folder + 'conv1_inter_incorrect_meta_train.torch'
    ...
    conv2_incorrect_valid_file = results_folder + 'conv2_inter_incorrect_meta_valid.torch'

    conv1_correct_valid_file = results_folder + 'conv1_inter_correct_meta_valid.torch'
    conv1_incorrect_valid_file = results_folder + 'conv1_inter_incorrect_meta_valid.torch'
    ...
    conv2_incorrect_test_file = results_folder + 'conv2_inter_incorrect_meta_test.torch'
    conv1_correct_test_file = results_folder + 'conv1_inter_correct_meta_test.torch'
    conv1_incorrect_test_file = results_folder + 'conv1_inter_incorrect_meta_test.torch'
    ...
    #makeMetaIntermediatesAfterTest(args, meta_model, device, error_test_loader, correct_test_loader, layer, mode)
    return meta_model, best_total_diff_adj_geo_acc_correct, best_total_diff_adj_geo_acc_error, test_correct_acc, test_error_acc
...
def main():
    ...
    parser.add_argument('--base_train_num_epochs', type=int, default=1, metavar='basetrainepochs',
                        help='size of batches to the meta classifier')
    parser.add_argument('--meta_train_num_epochs', type=int, default=25, metavar='metatrainepochs',
                        help='size of batches to the meta classifier')
    parser.add_argument('--load_base_model_from_saved_state', default="")
    parser.add_argument('--load_meta_model_from_saved_state', default="")
    

    meta_sizes_accuracies_file_name = results_folder+sys.argv[0]+'meta_acc_by_num_base_train_and_layer_accuracies_record.txt'
    ...
    for x in [1,2,3,4,5,10]:
        ...
        accuracies_file.write(str(x) + " " + "base acc" + str(base_acc) + '\n')
        meta_train_sampler = SubsetRandomSampler(meta_full_train_idx)

        meta_validation_sampler = SubsetRandomSampler(meta_full_valid_idx)
        ...
        #for l in ['fc1', 'output', 'conv1', 'conv2']:

        for l in ['output']:
            accuracies_file.write(str(x) + " " + l + '\n')
            print("make and train meta model _____________________")
            meta_model, correct, error, test_correct, test_error = make_and_train_meta_model(args, device, train_set_percentage=1, layer=l)
            meta_sizes_accuracies_file.write(str(x) + " " + l + " " + str(correct) + " " + str(error)+" " +str(test_correct)+" " +str(test_error)+'\n')
