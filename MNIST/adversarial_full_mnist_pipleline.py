...
results_folder = '../../results/'+name_and_time+'/'
...
def makeAdvIntermediates(args, model, device, mode):
    model.eval()
    ...
    with torch.no_grad():
        ...
        test_loss = 0
        correct = 0
        all_data = pickle.load(open('../../adv-meta/10k_test_images.pkl', "rb"))

        adv_examples = []
        adv_targets = []
        for item in all_data:
            adv_examples.append(item[0])
            adv_targets.append(item[4])

        adv_targets = torch.tensor(adv_targets)
        adv_examples = np.asarray(adv_examples)
        adv_examples = adv_examples.reshape((10000, 1, 28, 28))
        ...
        for i in list(range(100)):
            ...
            data = adv_examples[i*100:i*100+100]
            target = adv_targets[i*100:i*100+100]
...
def main():
    ...
    for x in [10]:
        ...
        makeAdvIntermediates(args, base_model, device, "test")
