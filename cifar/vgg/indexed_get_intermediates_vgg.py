...
import sys
base_path = '/media/chad/paro/'
saved_model = '/media/chad/nara/vgg-cifar-bn/runs/vgg-cifar100_bnmodel_best.pth62.tar'
...
hooked_layers = [Layer('conv_0', 'output', lambda model: list(model.children())[0][0]),
                 Layer('conv_18', 'output', lambda model: list(model.children())[0][18]),
                 Layer('conv_37', 'output', lambda model: list(model.children())[0][37]),
                 Layer('conv_37_post_bn', 'output', lambda model: list(model.children())[0][38]),
                 Layer('fc_0', 'output', lambda model: list(model.children())[1][0]),
                 Layer('fc_3', 'output', lambda model: list(model.children())[1][3])]
...
def main():
    ...
    train_dataset = datasets.CIFAR100('~/Dropbox/meta/data', train_indices, train=True, transform=transform_test, download=True)
    valid_dataset = datasets.CIFAR100('~/Dropbox/meta/data', valid_indices, train=True, transform=transform_test, download=True)
    ...
    saved_state = torch.load(saved_model)
...
def generate_intermediate_outputs(val_loader, model, criterion, epoch):
    ...
    for i, (indices, input, target) in enumerate(val_loader):
        ...
        save_tensor(correct_outputs, base_path + '/vgg16lesstrained/valid/output/correct/correct_output' + str(i) + '.torch')
        save_tensor(incorrect_outputs,
                    base_path + '/vgg16lesstrained/valid/output/incorrect/incorrect_output' + str(i) + '.torch')

        for j, layer in enumerate(hooked_layers):
            save_tensor(correct_layer_hooks[j], base_path + '/vgg16lesstrained/valid/' + layer.name + '_layer/correct/correct_inter_output' + str(i) + '.torch')
            save_tensor(incorrect_layer_hooks[j], base_path + '/vgg16lesstrained/valid/' + layer.name + '_layer/incorrect/incorrect_inter_output' + str(i) + '.torch')
        # clear hook lists
        for layer in hooked_layers:
            layer.hook_list[:] = []
